...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--retry-count", type=int, default=3, help="test_id 실패 시 최대 재시도 횟수")
    parser.add_argument("--retry-sleep-seconds", type=int, default=60, help="재시도 전 대기 시간(초)")
    return parser
...
def main() -> int:
    ...
    for i, (test_case, test_id, test_id_dir) in enumerate(targets, start=1):
        ...
        try:
            ...
            for retry_attempt in range(1, args.retry_count + 1):
                try:
                    asyncio.run(
                        _run_single_index(
                            repo_root=repo_root,
                    ...
                    last_error = str(exc)
                    print(
                        f"[WARN] {test_case}/{test_id} retry {retry_attempt}/{args.retry_count} failed: {last_error}"
                    )
                    if retry_attempt < args.retry_count:
