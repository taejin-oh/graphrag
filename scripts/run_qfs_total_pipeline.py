...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="query policy 순서(쉼표 구분). 기본/지원: default",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="각 index/query 단위 완료 직후 대기 시간(초). 기본 60",
    )
    parser.add_argument(
        "--max-rounds",
        type=int,
...
def main() -> int:
    ...
    unsupported = [policy for policy in policies if policy not in DEFAULT_POLICIES]
    if unsupported:
        raise ValueError(
            f"Unsupported policies requested: {unsupported}. Supported: {DEFAULT_POLICIES}"
        )
